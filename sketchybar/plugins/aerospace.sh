...
# Handle mouse events only - optimized aerospace call
if [ "$SENDER" = "mouse.entered" ]; then
  sketchybar --set $NAME background.color=$WORKSPACE_HOVER_BG \
                           label.color=$HOVER_TEXT \
                           icon.color=$HOVER_TEXT
elif [ "$SENDER" = "mouse.exited" ]; then
  # Determine current state and restore - optimized with timeout
  CURRENT_WORKSPACE=$(timeout 1s aerospace list-workspaces --focused 2>/dev/null)
